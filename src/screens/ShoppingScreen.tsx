...
function localizeShoppingSourceText(source: string, language: string) {
  ...
  return String(source || '')
    ...
    .map((part) => {
      ...
      const normalizedPart = part.replace(/\s+Breakfast:/i, ' · Breakfast:')
        .replace(/\s+Lunch:/i, ' · Lunch:')
        .replace(/\s+Dinner:/i, ' · Dinner:')
        .replace(/\s+Snack:/i, ' · Snack:');
      const flexibleMatch = match || normalizedPart.match(/^(\d{4}-\d{2}-\d{2})\s*[·路]\s*([^:]+):\s*(.+)$/);
      if (!flexibleMatch) return localizeEnglishMealText(part, language);

      const [, dateKey, slot, mealName] = flexibleMatch;
      const slotKey = slot.trim();
      const normalizedSlotKey = slotKey.charAt(0).toUpperCase() + slotKey.slice(1).toLowerCase();
      const localizedSlot = slotLabels[language]?.[slotKey] || slotLabels[language]?.[normalizedSlotKey] || slot.trim();
      const localizedMealName = localizeEnglishMealText(mealName, language);
      return `${dateKey} · ${localizedSlot}: ${localizedMealName}`;
    })
    .join(', ');
}
...
function localizeShoppingMeasureText(value?: string | null, language = 'en') {
  const raw = String(value || '').trim();
  if (!raw || language === 'en') return raw;
...
function formatSummedGramQuantity(value?: string | null, language = 'en') {
  const raw = String(value || '').trim();
  if (!raw) return '';
  ...
  if (parts.length <= 1) {
    return localizeShoppingMeasureText(raw, language);
  }
  ...
  return localizeShoppingMeasureText(`${formatted}g`, language);
}

function formatShoppingQuantity(value?: string | null, language = 'en') {
  const raw = String(value || '').trim();
  if (!raw) return '';
  ...
  const grams = parts.map((part) => {
    const match = part.replace(/,/g, '.').replace(/\s+/g, '').match(/^(\d+(?:\.\d+)?)(?:g|克)$/i);
    return match ? Number(match[1]) : NaN;
  });

  if (parts.length > 1 && grams.every((value) => Number.isFinite(value))) {
...
function getShoppingItemQuantity(item: ShoppingItem, language: string) {
  ...
  return formatShoppingQuantity(rawQuantity, language);
}

function getShoppingItemSource(item: ShoppingItem, language: string) {
  if (language === 'zh') {
    return localizeShoppingSourceText(item.sourceCn || item.sourceEn || item.source, language);
  }

  if (language === 'ms') {
    return localizeShoppingSourceText(item.sourceMs || item.sourceEn || item.source, language);
  }
